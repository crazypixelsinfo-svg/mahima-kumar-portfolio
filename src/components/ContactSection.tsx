...
const ContactSection = ({ compact }: ContactSectionProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("https://formspree.io/f/bellamkondamahimakumar@gmail.com", {
        method: "POST",
        body: formData,
      ...
      if (response.ok) {
        ...
        toast({ title: "Message Sent", description: "Thank you! Your message has been sent." });
  ...
  const contactInfo = [
    { icon: Phone, label: "Phone", value: "+91 9392429522", href: "tel:+919392429522" },
    { icon: MapPin, label: "Location", value: "Masab Tank, Hyderabad, Telangana", href: null },
    { icon: Linkedin, label: "LinkedIn", value: "Connect on LinkedIn", href: "https://www.linkedin.com/in/bellamkonda-mahima-kumar" },
  ];

  return (
    <section ref={sectionRef} id="contact" className="section-padding bg-card/50 grain">
      <div className="max-w-4xl mx-auto">
        <div className={`text-center mb-16 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          <p className="text-muted-foreground max-w-xl mx-auto">Open for freelance projects, collaborations, and full-time opportunities</p>
        </div>
        ...
        {!compact && (
          <div className={`transition-all duration-1000 delay-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            <form ref={formRef} onSubmit={handleSubmit} className="bg-secondary/20 border border-border rounded-2xl p-6 md:p-8 space-y-6">
              ...
              {submitStatus === "success" && (
                <div className="flex items-center gap-2 text-green-400 bg-green-400/10 border border-green-400/20 rounded-lg p-4">
                  <CheckCircle className="w-5 h-5" /><span>Thank you! Your message has been sent.</span>
                </div>
              )}
              {submitStatus === "error" && (
                <div className="flex items-center gap-2 text-red-400 bg-red-400/10 border border-red-400/20 rounded-lg p-4">
                  <AlertCircle className="w-5 h-5" /><span>Failed to send message. Please try again.</span>
                </div>
              )}
              <Button type="submit" variant="glow" size="lg" className="w-full md:w-auto" disabled={isSubmitting}>
                {isSubmitting ? <span className="animate-pulse">Sending...</span> : <><Send className="mr-2 h-4 w-4" />Send Message</>}
              </Button>
            </form>
          </div>
        )}
